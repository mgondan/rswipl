...
//
// NULL -> []
// numeric vector of length 1 -> real
// numeric vector of length > 1 -> e.g. #(1.0, 2.0, 3.0)
...
PlTerm r2pl_compound(Language r, CharacterVector& names, PlTerm& vars)
{
...
{
  PlTermv fun(2) ;
  fun[1] = r2pl_compound(BODY(r), names, vars) ;
...
LogicalVector init_(String argv0)
{
  ...
  if(!PL_initialise(argc, (char**) argv))
    stop("rswipl: initialization failed.") ;
...
LogicalVector done_()
{
  if(!pl_initialized)
  {
    warning("rswipl: swipl has not been initialized") ;
    return true ;
  }
